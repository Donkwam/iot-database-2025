...
-- GROUP BY를 사용하면 반드시 집계함수 및 GROUP BY에 포함된 컬럼이 SELECT 안에 포함시켜야함.
...
-- 3-20 가격이 8000원 이상인 도서를 구매한 고객에 대하여 고객별 주문도서의 총 수량을 구하시오.
...
-- 3-21 : 고객과 고객의 주문에 관한데이터를 모두 나타내시오.
SELECT *
  FROM Customer
 INNER JOIN Orders
...
SELECT c.name
  ...
  FROM Customer AS c, Orders AS o, Book AS b
 WHERE c.custid = o.custid
   AND b.bookid = o.bookid -- JOIN을위한 조건
...
-- 2-29 도서를 구매한 적이 있는 고객의 이름을 검색하시오.
SELECT c.name	
  FROM Customer AS c
 WHERE c.custid IN (
...
SELECT * 
 ...
 WHERE EXISTS (
			SELECT *					-- o.custid로 사용하지 않아도 됨
			  FROM Orders AS o
			 WHERE o.custid = c.custid	-- 메인쿼리에 컬럼이 서브쿼리에 사용되고 있음
			);