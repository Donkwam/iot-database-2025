...
SELECT bookid
	 , REPLACE(bookname, '야구', '농구') AS booknameBook
     , publisher
     , price
  FROM Book;
...
SELECT bookname AS '제목'
	 , CHAR_LENGTH(bookname) AS '제목 문자수'
...
-- 날짜시간 함수
SELECT SYSDATE(); -- Docker 서버시간을 따라서 GMMT(그리니치 표준시)를 따름 +9hour
...
-- 4-8 2014년 7월 7일에 주문받은 도서의 주문번호, 고객번호, 도서번호를 모두 나타내시오.
