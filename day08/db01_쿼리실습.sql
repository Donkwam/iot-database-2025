...
select concat(e1.first_name, ' ', e1.last_name) as 'name'
 ...
 where (e1.job_id, e1.salary) in (select job_id
...
-- 조건부 논리 표현식 제어 : CASE
/* 심플문제1 -프로젝트 성공으로 급여인상이 결정됨
	사원현재 107명 19개 업무에 소속되어 근무중. 회사 업무 Distince job_id 5개 업무에서 일하는 사원.
	HR_REP(10), MK_REP(12), PR_REP(15), SA_REP(18), IT_PROG(20%) 5개 업무를 제외하고는 나머지 동결(107개 행)
*/
select employee_id
...
/* 문제3 - 월별로 입사한 사원수가 아래와 같이 행렬로 출력되도록 하지오. 12행
*/
-- 형변환 함수 cast(), convert()
...
/* 문제1 - 이전문제를 활용, 집계결과가 아니면(ALL-DEPTs)라고 출력, 업무에 대한 집계결과가 아니면 (ALL-JOBs)를 출력
			rollup으로 만들어진 소재면 (ALL-JOBs), 총계면 (ALL-DEPTs)
*/
...
select employee_id
     ...
     , dense_rank() over (partition by department_id order by  salary desc) as 'Dense_Rank'	-- 1,1,2 순위 매기기 dense_rank
