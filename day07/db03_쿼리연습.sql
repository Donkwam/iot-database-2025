...
/* 생플 - Employee에서 사원번호, 이름, 급여, 업무, 입사일, 상시의 사원번호를 출력하시오.
...
/* 문제1 - employee 에서 사원의 성과 이름을 Name, 업무는 Job, 급여는 Salary, 연봉에 $100보너스를 추가해서 계산한 Increased Ann_Salary
...
/* 문제2 - Employees 에서 모든 사원의 last_name과 연봉을 '이름 : 1 Year Salry = $연봉' 형식으로 출력하고, 1 Year Salary라는 별칭을 붙이세요.(107해)
*/
...
/* 문제2 - 현재 날짜 타입을 날짜함수를 통해 확인, 1995년 5월 20일 ~ 1996년 5월 20일 사이 입사한 사원의 이름Name으롭 별치
			사원번호, 고용일지를 출력하시오. 단, 입사일이 빠른 순으로 정렬하시오.(8행)
*/
...
select last_name as 'name'
     ...
     , hire_date
  from employees
 where hire_date between '1995-05-20' and '1995-05-20'	-- date타입은 문자열처럼 조건만해도됨
 order by hire_date asc;
 
-- 단일행 함수 및 변환 함수
/* 문제1 - 이름이 s로 시작하는 각 사원의 업무를 아래의 예와 같이 출력하고자 함.(18행)
...
/* 문제3 - 사원의 성과 이름을 Name으로 별칭, 입사일, 입사한 요일을 출력하시오. 이떄 주(week) 시작인 일요일부터 출력되도록 정렬(107행)
*/
select concat(first_name, ' ', last_name) as 'name'
	 , hire_date
...
-- 집계함수
/* 문제1 - 사원이 소속된 부서별 급여 합계, 급여평균, 급여 최대값을 집계
...
select department_id
     ...
     , concat('$',format(round(avg(salary), 1), 1)) as 'Aug Salary'	 -- round(값, 1), 소수점 1자리에서 반올림, format(값, 1)소수점표현 및 1000단위, 표시
     , concat('$',format(round(max(salary), 0), 0)) as 'Max Salary'
     , concat('$',format(round(min(salary), 0), 0)) as 'Min Salary'
  from employees
 where department_id is not null
 group by department_id;	-- 그룹바이 속한 컬럼만 select절에 사용할수 있음!
 
-- 조인
/* 문제2 - job_grades 테이블 사용, 각 사원의 급여에 따른 급여등급을 보고한다. 이름과 성을 name으로, 업무, 부서명, 입사일, 급여, 급여등급을 출력(106행)
*/
desc job_grades;
desc employees;
...
-- 서브쿼리
/* 문제3 - 사원들의 지역별 근무현황을 조회. 이름이 영문'0'로 시작하는 지역에 살고
