...
def showDatas():
    ...
    ### DB연결. 커넥션 객체생성 -> 커서 -> 쿼리생성 -> 커서로 데이터 패치 -> 커서 종료 -> 커넥션 종료
    ...
    for i, (std_id, std_name, std_mobil, std_regyear) in enumerate(data, start=1):  # enumerate - 인덱스와 아이템 동시접근
        dataView.insert('', 'end', values=(std_id, std_name, std_mobil, std_regyear))   # 트리뷰 마지막 'end' 추가

    cursor.close()  # 커서 종료
    conn.close()    # 커넥션 종료

# 6. getData(envent) 트리뷰 한줄 더블 클릭한 값 엔트리에 표시
def getData(event):
    '''
    트리뷰 더블클릭으로 선택된 학생정보를 엔트디 위젯에 채우는 사용자함수
...
def addData():
    ...
    try:
        conn.begin()    # befin transaction. 트랜잭션 시작
        # 쿼리 작성
...
def delData():
    ...
    if std_id == '':
            messagebox.showwarning('UPDATE','삭제 데이터를 선택요망!')
            return
    ...
    try:
        ...
        print(e)
        conn.rollback() # 트랜잭션 롤백
        messagebox.showerror('DELET', '학생삭제 실패!!')
    finally:
        cursor.close()  # 커서
        conn.close()   # 커넥션 종료
...
root.resizable(False, False)    # 윈도우 사이즈 변셩 불가
