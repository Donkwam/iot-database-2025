...
- SQL 기초
    - 기본 데이터형
        ...
        - 주요 데이터형
            ...
            - **Int(4)** - 모든 데이터타입의 기준! 정수(음수)를 저장
            - BigInt(8) - Int보다 더 큰수 저장
            - Float(4) - 소수점아래 7자리까지 저장
            - Decimal(5~17byte) - Float 보다 더 큰 수 저장시
            ...
            - Longtext(최대 4GB) - mp3, mp4 음악, 영화데이터 자체 저장할 때 사용
...
- SubQuery 고급 : [SQL](./day04/db02_sql고급_서브쿼리.sql)
    - WHERE절 - 단일값(비교연산), 다중행(ALL|ANY|EXISTS|IN|NOR IN ...)
    - SELECT절
    - FROM절
- SQL 고급
    - 뷰 : [SQL](./day04/db03_sql고급_뷰.sql)
        - 자주 사용할 쿼리로 만들어진 가상 테이블을 계속 사용하기 위해서 만든개체
        - 입력, 수정도 가능. 조인된 뷰는 불가능
        - 보안적, 재사용성, 독립성을 위해서 사용
    - 인텍스 [SQL](./day04/db04_sql고급_인덱스.sql)
        - 빠른 검색을 위해서 사용하는 개체
...
- 데이터베이스 프로그래밍
    - 저장 프로시저 : [SQL](./day04/db05_저장프로시저1.sql)
        ...
        - 저장프로시저 하나로 프로그램 구현시 코드가 매우 짧아짐
...
- 데이터모델링
    ...
    - DB 생명주기
        ...
        5. 감시 및 개선
    - 모델링 순서
        1. 개념적 모델링 - 요구사항 수집 분석 내용 토대로 러프하게 전체 뼈대릴 새우는 과정
    ...
    - ERWin 설계 실습 : [ERWin](./day05/db06_ERWorkbance_skima.sql)
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 정규화 - 이상현상이 발생하지 않도록 릴레이션(개체, 테이블)을 분해하는 과정
        ...
        - BCNF정규화 - 함수종속성 A → B가 성립시 모든 결정자가 A가 후보키가 되는 정규화(무손실 분해)
        - 제4정규화 - 실무에서 사용안함. 다치종속성 릴레이션 가진 정규화
        - 제5정규화 - 실무에서 사용안함. 프로젝트-조인 정규형. 조인종속성을가진 릴레이션
- 트랜잭션 : [SQL](./day06/db04_트랜잭션.sql)
    - 데이터를 다루는 논리적인 작업단위
    - START TRANSACTION, SAVEPOINT, ROLLBACK, [TO SAVEOPINT], COMMIT 트랜잭션 처리
    - 특징
...
- Workbench Tip
    ...
    - 성능테스트 시 1000개 재한을 풀어줘야 함
    - 메뉴 `Edit > Preferences > SQL Editer > SQL Execution 에서 Limit Rows Count`를 조절
...
- 데이터모델링 실습
    - 병원업무관리
        - 요구사항으로 개체와 관계를 정입. 개체에 속하는 속성들, 식별자 결정
        - 테이블 명세서 작성(엑셀, 워드)
        - ERwin | Workbench 모델링에서 ERD 작성
        - 생성스크립트 : [SQL](./day08/db03_병원업무관리_스키마.sql)
        - Workbence에서 DB생성 후 위 스크립트 실행. DB구현
...
- tkinter DB연동 GUI앱 개발 : [Python](./day09/students_regapp.py)
    ...
    2. madang DB에 students 테이블 생성 및 더미데이터 추가
        ```sql
        drop table if exists student; -- 현재 테이블 삭제
