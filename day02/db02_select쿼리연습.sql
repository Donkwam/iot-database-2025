...
-- 파이썬, C#등 프로그래밍언어로 가져갈땐, 칼럼이름 칼럼갯수를 모두 파악해야 하기때문에 아래와 같이 사용
...
-- % 갯수와 상관없이 글자가 포함되는 것
-- []은 Oracle, SQL Server
-- [0-5] 1개의 문제가 일치, 한글자가 0에서 5의 숫자를 포함한다.
-- [^] 1개의 문자가 일치하지 않는 것
-- _ 특정위치의 1개의 문자가 일치할때 
-- 		_구% : 첫번째 글자가 뭐든지 상관없고, 두번째 글자가 구로되고 뒤에 글이 더있다.
SELECT *
  FROM Book
...
-- 3-10 : 축구에 관한 도서중 가격이 20000원 이상인 도서를 검색하시오.
SELECT *
  FROM Book
